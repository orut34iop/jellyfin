...
public class ResponseTimeMiddlewareTests
{
    ...
    public async Task Invoke_WhenResponseIsSlow_LogsWarningWithoutQueryString()
    {
        ...
        var responseTask = Task.Run(() => client.GetAsync("/Auth/Keys/path-secret?api_key=query-secret", TestContext.Current.CancellationToken), TestContext.Current.CancellationToken);
        try
        {
            Assert.True(requestEntered.Wait(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken));
            var completedTask = await Task.WhenAny(warningLogged.Task, Task.Delay(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken));
