...
public sealed class PeopleRepositoryTests : IDisposable
{
    ...
    public PeopleRepositoryTests()
    {
        ...
        _repository = new PeopleRepository(factory.Object, new Mock<IItemTypeLookup>().Object, Mock.Of<IItemQueryHelpers>());
    ...
    {
        var result = _repository.GetPeople(new InternalPeopleQuery { ItemId = _itemId, EnableTotalRecordCount = false });
    ...
    public void GetPeople_WithItemFilters_AppliesFiltersBeforeLimit()
    {
        ...
        var person = Assert.Single(result.Items);
    ...
    public void GetPeople_WithExcludedPersonType_ExcludesMatchingPeople()
    {
        ...
        Assert.DoesNotContain(result.Items, person => person.Type == PersonKind.Director);
    }
