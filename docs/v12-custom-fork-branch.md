...
2026-09-12 已将官方 `release-12.z` 合并至 `3c698bab7fb7c607e726a0bf79f0bb0f7616e962`。合并前的定制状态保存在本地备份分支 `backup/v12-custom-before-release-sync-20260912`。

这次同步包含官方 V12 维护修复，例如媒体探测失败重试、合集文件夹识别、剧集版本判断、Trickplay 流选择、媒体路径判断、家长分级、TMDb 图片尺寸、元数据提供者顺序和 FFmpeg 能力探测。详细结果见 [V12 定制迁移记录](v12-customization-migration.md)。
...
本次 `release-12.z` 同步及 PR 修订归并后，相关测试 619 项通过，13 项 Windows 专属测试在 macOS 跳过；`Jellyfin.sln` Release 构建为 0 警告、0 错误。安装版本为 `12.0.0-20260912215910`，Web 与健康检查返回 HTTP 200，媒体库扫描任务为 Idle，启动器完成一次优雅退出和重新启动。
