...
Jellyfin server backend: a cross-platform .NET 10 ASP.NET Core media server descended from Emby 3.5.2. It exposes a REST API for libraries, metadata, streaming, live TV, sessions, and plugins, and serves the static [jellyfin-web](https://github.com/jellyfin/jellyfin-web) client by default. The web client is **not** in this repo — obtain it from a published build, an existing install, or build it from `jellyfin-web` separately.

Prerequisites: [.NET 10.0 SDK](https://dotnet.microsoft.com/download/dotnet) and [ffmpeg](https://github.com/jellyfin/jellyfin-ffmpeg) on `PATH` (or pass `--ffmpeg` at startup).

## Common commands

Build everything / build server only:
...
Run the server (web client required, or use `--nowebclient`):
...
Migrations are provider-specific (SQLite is the only supported provider currently). From repo root:
...
If `dotnet ef` is unavailable, run `dotnet tool restore` first.

## High-level architecture

### Layer/project layout

Three broad layers; avoid introducing circular project references.
...
- Authentication uses a custom `CustomAuthenticationHandler` and authorization policies under `Jellyfin.Api.Auth`.
- Middleware lives in `Jellyfin.Api.Middleware` and is wired in `Jellyfin.Server.Startup.Configure`.
- Swagger/ReDoc at `/api-docs/swagger` and `/api-docs/redoc`.
- API supports both camelCase and PascalCase JSON via custom output formatters.
...
- Debug builds enable `AnalysisMode=AllEnabledByDefault` and run analyzers:
  ...
  - Custom analyzer project: `src/Jellyfin.CodeAnalysis`
