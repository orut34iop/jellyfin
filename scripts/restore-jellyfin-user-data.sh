...
case "$OS_NAME" in
    Darwin)
        DEFAULT_DATA_DIR="$HOME/Library/Application Support/jellyfin-v12"
        DEFAULT_CONFIG_DIR=""
        DEFAULT_BACKUP_ROOT="/Volumes/mba2t/backup/jellyfin-v12"
        ;;
    Linux)
...
JELLYFIN_PROCESS_PATTERN='[/]Applications/Jellyfin V12[.]app/Contents/'
...
stop_jellyfin() {
    ...
    echo "Stopping Jellyfin V12.app before restore"
    find_jellyfin_pids | while read -r pid; do
        [ -z "$pid" ] || kill -TERM "$pid"
    done
...
restart_jellyfin() {
    if [ "$OS_NAME" = "Linux" ]; then
        ...
        open "/Applications/Jellyfin V12.app"
    fi
}
