...
This is the Jellyfin server backend: a cross-platform .NET 10 ASP.NET Core media server descended from Emby 3.5.2. It exposes a REST API for libraries, metadata, streaming, live TV, sessions, and plugins, and serves the static [jellyfin-web](https://github.com/jellyfin/jellyfin-web) client by default.
...
- On macOS arm64, after any code change, rebuild the macOS arm64 app, install it to `/Applications/Jellyfin V12.app`, and verify it starts successfully. The minimum verification is opening `/Applications/Jellyfin V12.app`, confirming `http://127.0.0.1:8096/web/index.html` returns HTTP 200, and confirming `/System/Info/Public` reports the timestamped version.
...
- macOS packaging must preserve the native menu-bar launcher, bundled application/status icons, service lifecycle controls, Web/log shortcuts, preferences, and Launch at Login support. HTTP-only checks are insufficient for desktop changes: run `packaging/macos/LauncherTests.swift` with `LauncherCore.swift`, check the installed launcher diagnostics and desktop entry points, verify graceful launcher/server shutdown and relaunch, and compare the desktop capability checklist in `docs/v12-customization-migration.md`. Do not enable login items or log out the user just to test them.
